import { CancelJobUseCase, GetAllJobsUseCase, GetJobStatusUseCase, SubmitJobUseCase } from "../../../application/usecases";
...
export class JobController {
  ...
  private listAllJobsUseCase: GetAllJobsUseCase;
  ...
  constructor(repository: JobRepository) {
    ...
    this.listAllJobsUseCase = new GetAllJobsUseCase(repository);
  }
  ...
  getAllJobs = async (req: Request, res: Response): Promise<void> => {
    try {
      const jobs = await this.listAllJobsUseCase.execute();
      const formattedJobs = jobs.map((job) => this.formatJobResponse(job));
